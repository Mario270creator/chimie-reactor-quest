...
- **Panou de administrare** (`/admin`, link ⚙️ Admin în meniu pentru administratori):
  - creezi, editezi, ștergi conturi; resetezi parole; acorzi drepturi de admin
  - editezi și ștergi lecții, teste (inclusiv întrebările), clase și anunțuri
  - statistici și istoric al încercărilor la teste, export CSV
- **Editor chimic cu indici** pe toate câmpurile de conținut (lecții, teste, anunțuri):
  - butoane X₂ / X² pentru indici și exponenți (selectezi cifrele → apeși butonul)
  - ✨ formatare automată: `2H2 + O2 -> 2H2O` devine `2H₂ + O₂ → 2H₂O`, `Ca^2+` devine `Ca²⁺`
  - simboluri rapide: → ⇌ ↑ ↓ Δ ° · și meniu cu formule uzuale (H₂SO₄, Ca(OH)₂ …)
  - scurtături: Ctrl+Shift+↓ (indice), Ctrl+Shift+↑ (exponent)
- **Schimbarea parolei proprii** din Panou principal → Contul meu (orice utilizator)
- Primul profesor din baza de date devine automat administrator (contul `profesor_demo` la instalare)
- Ghid complet de publicare și editare pe GitHub: vezi `GHID_GITHUB.md`
