...
@admin_required
def admin_dashboard():
    # Extragem toți utilizatorii cu excepția adminului curent ca să nu se șteargă din greșeală
    users = query_all("SELECT id, full_name, username, role FROM users WHERE id != ?", (session["user_id"],))
    
    # Transformăm rândurile SQLite în formatele exacte cerute de template (u[0], u[1], u[2], u[3])
    users_list = [(u["id"], u["full_name"], u["username"], u["role"]) for u in users]
...
@app.post("/admin/change_role/<int:user_id>")
@admin_required
def change_role(user_id: int):
    new_role = request.form.get("new_role", "").strip()
    if new_role not in ["elev", "profesor", "admin"]:
...
def make_me_admin():
    try:
        execute_db("UPDATE users SET role = 'admin' WHERE id = ?", (session["user_id"],))
        flash("Gata, șefule! Acum ești admin. Butonul roșu din meniu este activ.", "success")
    except Exception as e:
        flash(f"Eroare (probabil din cauza unui cache vechi al bazei de date): {str(e)}. Încearcă să ștergi fișierul instance/chimie_reactor_quest.db local pentru a forța regenerarea tabelelor cu noul format.", "danger")
    return redirect(url_for("dashboard"))
