...
def run_migrations(conn: sqlite3.Connection) -> None:
    ...
    if "is_admin" not in columns:
        ...
        conn.commit()
    # Ensure at least one administrator exists: promote the oldest teacher.
    has_admin = conn.execute("SELECT id FROM users WHERE is_admin = 1 LIMIT 1").fetchone()
    if has_admin is None:
        oldest_teacher = conn.execute(
            "SELECT id FROM users WHERE role = 'profesor' ORDER BY id ASC LIMIT 1"
        ).fetchone()
        if oldest_teacher is not None:
            conn.execute("UPDATE users SET is_admin = 1 WHERE id = ?", (oldest_teacher[0],))
            conn.commit()
...
def admin_required(view):
    ...
    def wrapped(*args, **kwargs):
        ...
        if not user_is_admin(user):
            flash("Zona de administrare este rezervată administratorului.", "danger")
            return redirect(url_for("dashboard"))
        return view(*args, **kwargs)
...
def change_own_password():
    ...
    if not check_password_hash(user["password_hash"], current_pw):
        flash("Parola actuală nu este corectă.", "danger")
    elif len(new_pw) < 4:
        flash("Parola nouă trebuie să aibă cel puțin 4 caractere.", "warning")
    elif new_pw != confirm_pw:
        flash("Parolele noi nu coincid.", "warning")
    else:
        execute_db(
...
def admin_overview() -> dict[str, Any]:
    counts = {
        "users": query_one("SELECT COUNT(*) AS c FROM users")["c"],
        "teachers": query_one("SELECT COUNT(*) AS c FROM users WHERE role = 'profesor'")["c"],
        "students": query_one("SELECT COUNT(*) AS c FROM users WHERE role = 'elev'")["c"],
        "classes": query_one("SELECT COUNT(*) AS c FROM classes")["c"],
        "lessons": query_one("SELECT COUNT(*) AS c FROM lessons")["c"],
        "quizzes": query_one("SELECT COUNT(*) AS c FROM quizzes")["c"],
        "attempts": query_one("SELECT COUNT(*) AS c FROM attempts")["c"],
        "announcements": query_one("SELECT COUNT(*) AS c FROM announcements")["c"],
    }
    return counts
...
def admin_panel():
    ...
    if q:
        users = query_all(
            "SELECT * FROM users WHERE full_name LIKE ? OR username LIKE ? ORDER BY id",
            (f"%{q}%", f"%{q}%"),
        )
    else:
    ...
    user_rows = []
    for u in users:
        level = compute_level(u["id"])
        user_rows.append({**dict(u), "xp": level["xp"], "level_title": level["title"]})
...
def admin_create_user():
    ...
    username = request.form.get("username", "").strip().lower()
    password = request.form.get("password", "")
    role = request.form.get("role", "elev").strip()
    make_admin = 1 if request.form.get("is_admin") == "on" else 0
    if role not in {"profesor", "elev"}:
        role = "elev"
    if len(full_name) < 3 or len(username) < 3 or len(password) < 4:
        flash("Completează nume (min. 3), utilizator (min. 3) și parolă (min. 4 caractere).", "warning")
        return redirect(url_for("admin_panel"))
    if query_one("SELECT id FROM users WHERE username = ?", (username,)):
        flash("Numele de utilizator există deja.", "danger")
        return redirect(url_for("admin_panel"))
    execute_db(
        "INSERT INTO users(full_name, username, password_hash, role, bio, is_admin, created_at) VALUES (?, ?, ?, ?, '', ?, ?)",
        (full_name, username, generate_password_hash(password), role, make_admin if role == "profesor" else 0, utcnow_iso()),
    )
    flash(f"Contul „{username}” a fost creat.", "success")
    return redirect(url_for("admin_panel"))
...
def admin_update_user(user_id: int):
    ...
    if target is None:
        flash("Utilizatorul nu există.", "danger")
        return redirect(url_for("admin_panel"))
    me = current_user()
    full_name = request.form.get("full_name", "").strip()
    username = request.form.get("username", "").strip().lower()
    ...
    if len(full_name) < 3 or len(username) < 3:
        flash("Numele și utilizatorul trebuie să aibă cel puțin 3 caractere.", "warning")
        return redirect(url_for("admin_panel"))
    duplicate = query_one("SELECT id FROM users WHERE username = ? AND id != ?", (username, user_id))
    if duplicate:
        flash("Numele de utilizator este deja folosit de alt cont.", "danger")
        return redirect(url_for("admin_panel"))
    # Never allow removing the last administrator.
    if user_is_admin(target) and not make_admin:
        other_admins = query_one("SELECT COUNT(*) AS c FROM users WHERE is_admin = 1 AND id != ?", (user_id,))["c"]
        if other_admins == 0:
            flash("Nu poți elimina drepturile ultimului administrator.", "danger")
            return redirect(url_for("admin_panel"))
    execute_db(
        "UPDATE users SET full_name = ?, username = ?, role = ?, is_admin = ? WHERE id = ?",
        (full_name, username, role, make_admin, user_id),
    )
    if user_id == me["id"] and not make_admin:
        flash("Ți-ai retras drepturile de administrator.", "warning")
        return redirect(url_for("dashboard"))
    flash("Contul a fost actualizat.", "success")
    return redirect(url_for("admin_panel"))
...
def admin_reset_password(user_id: int):
    ...
    if len(new_pw) < 4:
        flash("Parola nouă trebuie să aibă cel puțin 4 caractere.", "warning")
        return redirect(url_for("admin_panel"))
    execute_db("UPDATE users SET password_hash = ? WHERE id = ?", (generate_password_hash(new_pw), user_id))
    flash(f"Parola pentru „{target['username']}” a fost resetată.", "success")
    return redirect(url_for("admin_panel"))
...
def admin_delete_user(user_id: int):
    me = current_user()
    if user_id == me["id"]:
        flash("Nu îți poți șterge propriul cont din panoul de administrare.", "danger")
        return redirect(url_for("admin_panel"))
    target = query_one("SELECT * FROM users WHERE id = ?", (user_id,))
    if target is None:
        flash("Utilizatorul nu există.", "danger")
    ...
    if user_is_admin(target):
        other_admins = query_one("SELECT COUNT(*) AS c FROM users WHERE is_admin = 1 AND id != ?", (user_id,))["c"]
        if other_admins == 0:
            flash("Nu poți șterge ultimul administrator.", "danger")
            return redirect(url_for("admin_panel"))
    execute_db("DELETE FROM users WHERE id = ?", (user_id,))
    flash(f"Contul „{target['username']}” a fost șters, împreună cu datele asociate.", "success")
    return redirect(url_for("admin_panel"))
...
def admin_edit_lesson(lesson_id: int):
    ...
    if lesson is None:
        flash("Lecția nu există.", "danger")
        return redirect(url_for("admin_panel"))
    if request.method == "POST":
        title = request.form.get("title", "").strip()
        summary = request.form.get("summary", "").strip()
        content = request.form.get("content", "").strip()
        difficulty = request.form.get("difficulty", lesson["difficulty"]).strip()
        class_id = request.form.get("class_id", str(lesson["class_id"])).strip()
        try:
            xp_value = max(10, min(200, int(request.form.get("xp", lesson["xp"]))))
        except ValueError:
            xp_value = lesson["xp"]
        if len(title) < 4 or len(summary) < 8 or len(content) < 20:
            flash("Titlul, rezumatul și conținutul sunt prea scurte.", "warning")
        elif not class_id.isdigit() or query_one("SELECT id FROM classes WHERE id = ?", (int(class_id),)) is None:
            flash("Alege o clasă validă.", "warning")
        else:
            execute_db(
                "UPDATE lessons SET class_id = ?, title = ?, summary = ?, content = ?, xp = ?, difficulty = ? WHERE id = ?",
                (int(class_id), title, summary, content, xp_value, difficulty, lesson_id),
            )
            flash("Lecția a fost salvată.", "success")
            return redirect(url_for("admin_panel"))
    classes = query_all("SELECT * FROM classes ORDER BY name, section")
    return render_template("admin_lesson_edit.html", lesson=lesson, classes=classes)
...
def admin_delete_lesson(lesson_id: int):
    ...
    flash("Lecția a fost ștearsă.", "success")
    return redirect(url_for("admin_panel"))


@app.route("/admin/quizzes/<int:quiz_id>/edit", methods=["GET", "POST"])
@admin_required
def admin_edit_quiz(quiz_id: int):
    quiz = query_one(
        """SELECT q.*, c.name AS class_name, c.section AS class_section
    ...
    if quiz is None:
        flash("Testul nu există.", "danger")
        return redirect(url_for("admin_panel"))
    if request.method == "POST":
        title = request.form.get("title", "").strip()
        description = request.form.get("description", "").strip()
        difficulty = request.form.get("difficulty", quiz["difficulty"]).strip()
        class_id = request.form.get("class_id", str(quiz["class_id"])).strip()
        questions_payload = request.form.get("questions_payload", "")
        try:
            xp_value = max(20, min(300, int(request.form.get("xp", quiz["xp"]))))
        except ValueError:
            xp_value = quiz["xp"]
        try:
            questions = parse_questions_payload(questions_payload)
        ...
        if len(title) < 4:
            flash("Titlul testului este prea scurt.", "warning")
        elif not class_id.isdigit() or query_one("SELECT id FROM classes WHERE id = ?", (int(class_id),)) is None:
            flash("Alege o clasă validă.", "warning")
        else:
            execute_db(
                "UPDATE quizzes SET class_id = ?, title = ?, description = ?, xp = ?, difficulty = ?, questions_json = ? WHERE id = ?",
                (
                    int(class_id),
                    title,
                    description or "Test actualizat",
                    xp_value,
                    difficulty,
                    json.dumps(questions, ensure_ascii=False),
                    quiz_id,
            ...
            flash("Testul a fost salvat.", "success")
            return redirect(url_for("admin_panel"))
    classes = query_all("SELECT * FROM classes ORDER BY name, section")
    questions = json.loads(quiz["questions_json"])
    return render_template("admin_quiz_edit.html", quiz=quiz, classes=classes, questions=questions)


@app.post("/admin/quizzes/<int:quiz_id>/delete")
@admin_required
def admin_delete_quiz(quiz_id: int):
    execute_db("DELETE FROM quizzes WHERE id = ?", (quiz_id,))
    flash("Testul a fost șters, împreună cu încercările asociate.", "success")
    return redirect(url_for("admin_panel"))
...
def admin_delete_class(class_id: int):
    ...
    flash("Clasa a fost ștearsă, împreună cu lecțiile, testele și înscrierile ei.", "success")
    return redirect(url_for("admin_panel"))
...
def admin_delete_announcement(announcement_id: int):
    ...
    flash("Anunțul a fost șters.", "success")
    return redirect(url_for("admin_panel"))
